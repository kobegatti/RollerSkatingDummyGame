...
void Shape::instanceDraw(const shared_ptr<Program> prog) const
{
	...
	// Draw
	CHECKED_GL_CALL(glDrawElementsInstanced(GL_TRIANGLES, (int)eleBuf.size(), GL_UNSIGNED_INT, (const void*)0, 56));
...
void Shape::calculateOriginTranslate() {
	translate_to_origin.x = -((min.x + max.x) / 2);
	translate_to_origin.y = -((min.y + max.y) / 2);
	translate_to_origin.z = -((min.z + max.z) / 2);
}